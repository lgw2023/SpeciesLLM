...
def _build_x_list_array(X_chunk):
    n_rows, n_cols = X_chunk.shape
    offsets = np.arange(0, (n_rows + 1) * n_cols, n_cols, dtype=np.int32)
    flat = np.ascontiguousarray(X_chunk, dtype="f4").ravel()
    return pa.ListArray.from_arrays(pa.array(offsets), pa.array(flat))


def _process_chunk(
    chunk_idx, z_arr, n_samples, chunk_size, normalization,
    obs_mapped, keep_cols, schema, out_parquet_dir, row_group_size,
):
    start = chunk_idx * chunk_size
    end = min(start + chunk_size, n_samples)

    X_raw = np.asarray(z_arr[start:end], dtype=np.float32)
    X_chunk = preprocess_count_matrix(X_raw, normalization)

    x_col = _build_x_list_array(X_chunk)
    arrays = {"X": x_col}
    for col in keep_cols:
        if col == "X":
            continue
        arrays[col] = pa.array(obs_mapped[col].values[start:end], type=pa.int64())

    table = pa.table(arrays, schema=schema)

    part_path = join(out_parquet_dir, f"part.{chunk_idx}.parquet")
    pq.write_table(table, part_path, row_group_size=row_group_size)

    return part_path, end - start
...
def process_species_dir(
    ...
    try:
        z_arr = zarr.open(zarr_store_path, mode="r")
        total_rows = z_arr.shape[0]
    except Exception as e:
        print(f"[ERROR] {sp_name}: load zarr failed: {e}")
        return

    n_samples = (total_rows // row_group_size) * row_group_size
    if n_samples == 0:
    ...
    keep_cols = [c for c in keep_cols if c in obs_mapped.columns or c == "X"]
    ...
    out_parquet_dir = join(species_output_path, "obs")
    os.makedirs(out_parquet_dir, exist_ok=True)
    print(f"[WRITE] {sp_name} → {out_parquet_dir}")

    n_chunks = (n_samples + chunk_size - 1) // chunk_size

    if chunk_threads <= 1 or n_chunks <= 1:
        part_paths = []
        total_rows_written = 0
        for chunk_idx in range(n_chunks):
            pp, nrows = _process_chunk(
                chunk_idx, z_arr, n_samples, chunk_size, normalization,
                obs_mapped, keep_cols, schema, out_parquet_dir, row_group_size,
            )
        ...
        with ThreadPoolExecutor(max_workers=chunk_threads) as ex:
            ...
            for fut in as_completed(futures):
                pp, nrows = fut.result()
                part_paths[futures[fut]] = pp
                total_rows_written += nrows

    if any(p is not None for p in part_paths):
        metadata_path = join(out_parquet_dir, "_metadata")
        metadata_collector = []
        for pp in part_paths:
            if pp is None:
                continue
            _pf = pq.ParquetFile(pp)
            metadata_collector.append(_pf.metadata)
        pq.write_metadata(schema, metadata_path, metadata_collector=metadata_collector)

    print(f"[DONE] {sp_name} ({total_rows_written} rows written)")
...
def parse_args():
    ...
    p.add_argument("--species-jobs",
        type=int,
        default=1,
        help="Pass-2: number of species dirs to process concurrently (threads). 1 = sequential (default).",
    )
    p.add_argument("--chunk-threads",
        type=int,
        default=DEFAULT_CHUNK_THREADS,
        help="Number of threads for chunk-level parallelism within a single species. "
             "Each thread reads a zarr slice, normalizes, and writes a parquet part independently. "
             f"Default: {DEFAULT_CHUNK_THREADS}.",
    )
    p.add_argument(
        "--skip-pass1",
        action="store_true",
...
def main():
    ...
    print(f"[CONFIG] species_jobs     = {args.species_jobs}")
    print(f"[CONFIG] chunk_threads    = {args.chunk_threads}")
