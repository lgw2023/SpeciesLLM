...
bool_arg_value() {
  ...
  case "$value" in
    1|true|yes|y|on)
      ...
      ;;
    0|false|no|n|off)
      # train_MNodes_torchrun_mfu_preindexparquet.py has a few type=bool args.
      # bool("") is False, while bool("false") is True.
      printf ""
      ;;
    *)
      printf "%s" "$1"
...
log "EXPERIMENT AMP_DTYPE=${AMP_DTYPE}, STATIC_GENE_DTYPE=${STATIC_GENE_DTYPE}, RUNTIME_ATTN_IMPLEMENTATION=${RUNTIME_ATTN_IMPLEMENTATION:-config}"
