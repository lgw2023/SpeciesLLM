...
class CellTokenEncoder(nn.Module):
    def __init__(self, num_embeddings: int,  ## ntoken
...
class BertSdpaSelfAttention(BERTSelfAttention):
    ...
    def forward(self, hidden_states, attention_mask=None, output_attentions: Optional[bool] = False) -> Tuple:
        ...
        attn_output = torch.nn.functional.scaled_dot_product_attention(query_layer,
            key_layer,
            value_layer,
...
class ExprDecoder(nn.Module):
    def __init__(self, d_in: int, n_embd: int, explicit_zero_prob: bool = False, ):
        super().__init__()
        self.fc = nn.Sequential(nn.Linear(d_in,
                                          n_embd),
            nn.LeakyReLU(),
            nn.Linear(n_embd,
                      n_embd),
        ...
        if explicit_zero_prob:
            self.zero_logit = nn.Sequential(nn.Linear(d_in,
                                                      n_embd),
                nn.LeakyReLU(),
                nn.Linear(n_embd,
                          n_embd),
...
class MVCDecoder(nn.Module):
    def __init__(self, dim_in: int, n_embd: int, arch_style: str = "inner product",
            query_activation: nn.Module = nn.Sigmoid, hidden_activation: nn.Module = nn.PReLU,
            explicit_zero_prob: bool = False, ) -> None:
        super().__init__()
        if arch_style in ["inner product", "inner product, detach"]:
            self.gene2query = nn.Linear(n_embd,
                                        n_embd)
            self.query_activation = query_activation()
            self.W = nn.Linear(n_embd,
                               dim_in,
                               bias=False)
            if explicit_zero_prob:
                self.W_zero_logit = nn.Linear(n_embd,
                                              dim_in)
        elif arch_style == "concat query":
            self.gene2query = nn.Linear(n_embd,
                                        64)
            self.query_activation = query_activation()
            self.fc1 = nn.Linear(n_embd + 64,
                                 64)
    ...
    def forward(self, cell_emb: torch.Tensor, gene_emb: torch.Tensor
        ...
        if self.arch_style in ["inner product", "inner product, detach"]:
            ...
            pred_value = torch.bmm(self.W(query_vecs),
                                   cell_emb).squeeze(2)
            if not self.explicit_zero_prob:
                return dict(pred=pred_value)
            ...
            cell_emb = cell_emb.unsqueeze(1).expand(-1,
                                                    gene_emb.shape[1],
                                                    -1)
            h = self.hidden_activation(self.fc1(torch.cat([cell_emb, query_vecs],
                                                          dim=2)))
...
class ClsDecoder(nn.Module):
    ...
    def __init__(self, n_embd: int, n_cls: int, n_layers: int = 3, activation: callable = nn.ReLU, ):
        super().__init__()
        self._decoder = nn.ModuleList()
        for i in range(n_layers - 1):
            self._decoder.append(nn.Linear(n_embd,
                                           n_embd))
            self._decoder.append(activation())
            self._decoder.append(nn.LayerNorm(n_embd))
        self.out_layer = nn.Linear(n_embd,
                                   n_cls)
...
class FeatureWiseMLPFusion(nn.Module):
    def __init__(self, output_dim: int = 640):
        super().__init__()
        self.dna_proj = nn.Sequential(nn.Linear(2560,
                                                1280),
            nn.GELU(),
            nn.LayerNorm(1280))
        self.norm_esm = nn.LayerNorm(1280)
        self.norm_desc = nn.LayerNorm(1280)
        # 对每个1280维位置独立处理（类似CNN的通道混合）
        self.mlp = nn.Sequential(nn.Linear(3,
                                           8),
            # 混合三个特征通道
            nn.GELU(),
            nn.Linear(8,
    ...
    def forward(self, esm_emb, desc_emb, dna_emb):
        dna_emb = self.dna_proj(dna_emb)
        esm_emb = self.norm_esm(esm_emb)
        desc_emb = self.norm_desc(desc_emb)
        x = torch.stack([esm_emb, desc_emb, dna_emb],
            dim=2)  # 形状变为 [num_genes, 3, 1280]
        batch, num_genes, _, feat_dim = x.shape
        x = x.view(-1,
                   3,
                   feat_dim)  # [batch*num_genes, 3, 1280]
        x = x.transpose(1,
                        2)
        x = self.mlp(x)
        x = x.view(batch,
...
class BERTConfig:
    ...
    do_mvc: bool = True
    do_cls: bool = False
    domain_specific_bn: Union[bool, str] = False
    do_adbc: bool = False  # whether to use adversarial domain adaptation, if True, then num_batch_labels is required
...
class BERTModel(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.domain_specific_bn is True or config.domain_specific_bn == "dsbn":
            use_affine = True if config.domain_specific_bn == "do_affine" else False
            print(f"Using domain specific BN with affine: {use_affine}")
            self.domain_specific_bn = DomainSpecificBatchNorm1d(config.hidden_size,
                config.num_batch_labels,
                eps=6.1e-5,
                use_affine=use_affine, )
        elif config.domain_specific_bn == "batchnorm":
            print("Using simple batchnorm instead of domain specific BN")
    ...
    def forward(self, src: torch.Tensor, values: torch.Tensor, esm_embeddings: torch.Tensor,
        ...
        embeddings = self.enhanced_fusion(esm_embeddings,
                                          desc_embeddings,
                                          dna_embeddings)  # (batch, seq_len, emb_size)
        ...
        if getattr(self,
            ...
            x = self.domain_specific_bn(x.permute(0,
                                                  2,
                                                  1),
                                        batch_label).permute(0,
                                                             2,
                                                             1)
        elif getattr(self,
                     'bn',
                     None) is not None:
            x = self.bn(x.permute(0,
                                  2,
                                  1)).permute(0,
                                              2,
                                              1)
    ...
    def _check_batch_labels(self, batch_labels: torch.Tensor) -> None:
        if self.use_cell_labels:
            ...
            raise ValueError(f"labels should be provided when 'self.use_batch_labels' is True")
...
class BERTForPreTraining(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.use_batch_labels:
            self.batch_encoder = CellTokenEncoder(config.num_batch_labels,
                                                  config.cell_hidden_size)
        if config.use_species_labels:
            self.species_encoder = CellTokenEncoder(config.num_species_labels,
                                                    config.cell_hidden_size)
        if config.use_tissue_labels:
            self.tissue_encoder = CellTokenEncoder(config.num_tissue_labels,
                                                   config.cell_hidden_size)
        if config.use_seqmethod_labels:
            self.seqmethod_encoder = CellTokenEncoder(config.num_seqmethod_labels,
                                                      config.cell_hidden_size)
        if config.use_disease_labels:
            self.disease_encoder = CellTokenEncoder(config.num_disease_labels,
                                                    config.cell_hidden_size)
        # Add sex and age encoders
        if config.use_sex_labels:
            self.sex_encoder = CellTokenEncoder(config.num_sex_labels,
                                                config.cell_hidden_size)
        if config.use_age_labels:
            self.age_encoder = CellTokenEncoder(config.num_age_labels,
                                                config.cell_hidden_size)

        cell_attributes = [config.use_batch_labels, config.use_species_labels, config.use_tissue_labels,
            config.use_seqmethod_labels, config.use_disease_labels, config.use_sex_labels, config.use_age_labels, ]
        # Expression Decocer acts as BertOnlyMLMHead
        self.decoder = ExprDecoder(config.hidden_size + config.cell_hidden_size if any(cell_attributes) else config.hidden_size,
            config.hidden_size,
            config.explicit_zero_prob, )
        if config.do_mvc:
            self.mvc_decoder = MVCDecoder(config.hidden_size + config.cell_hidden_size if any(cell_attributes) else config.hidden_size,
                config.hidden_size,
                arch_style=config.mvc_decoder_style,
                explicit_zero_prob=config.explicit_zero_prob, )
        if config.do_cls:
            self.cls_decoder = ClsDecoder(config.hidden_size + config.cell_hidden_size if any(cell_attributes) else config.hidden_size,
                config.num_cls)

        if config.do_adbc:
            if config.num_batch_labels is None:
                raise ValueError("num_batch_labels is required when do_adbc is True")
            self.grad_reverse_discriminator = AdversarialDiscriminator(config.hidden_size + config.cell_hidden_size if any(cell_attributes) else config.hidden_size,
    ...
    def _get_cell_emb_from_layer(self, layer_output: torch.Tensor, weights: torch.Tensor = None, ) -> torch.Tensor:
        if self.config.cell_emb_style == "cls":
            ...
            cell_emb = torch.mean(layer_output,
                                  dim=1)
            ...
            cell_emb = torch.sum(layer_output * weights.unsqueeze(2),
                                 dim=1)
            cell_emb = F.normalize(cell_emb,
                                   p=2,
                                   dim=1)  # shape [batch, embsize]
        return cell_emb
    ...
    def _encode(self, src: Optional[torch.Tensor] = None, values: Optional[torch.Tensor] = None,
        ...
        cell_token_embs = []
        cell_attributes = [(self.config.use_batch_labels, batch_labels, self.batch_encoder if hasattr(self,
                                                                                                      'batch_encoder') else None),
            (self.config.use_species_labels, species_labels, self.species_encoder if hasattr(self,
                                                                                             'species_encoder') else None),
            (self.config.use_tissue_labels, tissue_labels, self.tissue_encoder if hasattr(self,
                                                                                          'tissue_encoder') else None),
            (self.config.use_seqmethod_labels, seqmethod_labels, self.seqmethod_encoder if hasattr(self,
                                                                                                   'seqmethod_encoder') else None),
            (self.config.use_disease_labels, disease_labels, self.disease_encoder if hasattr(self,
                                                                                             'disease_encoder') else None),
            (self.config.use_sex_labels, sex_labels, self.sex_encoder if hasattr(self,
                                                                                 'sex_encoder') else None), (
            self.config.use_age_labels, age_labels, self.age_encoder if hasattr(self,
                                                                                'age_encoder') else None), ]
        ...
        if cell_token_embs:
            cell_token_embs = torch.stack(cell_token_embs,
                                          dim=0).sum(dim=0)
            x = torch.cat([sequence_output, cell_token_embs.unsqueeze(1).repeat(1,
        ...
        cell_emb = self._get_cell_emb_from_layer(sequence_output,
                                                 values)
        cell_emb = torch.cat([cell_emb, cell_token_embs],
                             dim=1) if isinstance(cell_token_embs,
                                                  torch.Tensor) else cell_emb
    ...
    def forward(self, src: Optional[torch.Tensor] = None, values: Optional[torch.Tensor] = None,
            ...
            MVC: bool = False, targets: torch.Tensor = None, output_attentions: Optional[bool] = None,
            output_hidden_states: Optional[bool] = None, ):
        x, cell_emb, bert_hidden_states, bert_attentions = self._encode(src=src,
            values=values,
            esm_embeddings=esm_embeddings,
            desc_embeddings=desc_embeddings,
            dna_embeddings=dna_embeddings,
        ...
        if self.config.do_adbc:
            outputs["adbc_output"] = self.grad_reverse_discriminator(cell_emb)
    ...
    def from_pretrained(cls, model_type, override_args=None):
        ...
        config_args = {
            'bert-base-cased': dict(num_hidden_layers=12,
                                    num_attention_heads=10,
                                    hidden_size=1280),  # 245M params
            'bert-large-cased': dict(num_hidden_layers=24,
                                     num_attention_heads=16,
                                     hidden_size=1536),  # 340M params
            }[model_type]
        ...
        transposed = ['attention.self.query.weight', 'attention.self.key.weight', 'attention.self.value.weight',
    ...
    def estimate_mfu(self, fwdbwd_per_iter, dt):
        ...
        flops_promised = 312e12  # A100 GPU bfloat16 peak flops is 312 TFLOPS, 910b NPU bfloat16 peak flops is 256 TFLOPS
...
class AdversarialDiscriminator(nn.Module):
    ...
    def __init__(self, d_model: int, n_cls: int, nlayers: int = 3, activation: callable = nn.LeakyReLU,
            reverse_grad: bool = False, ):
        super().__init__()
        # module list
        self._decoder = nn.ModuleList()
        for i in range(nlayers - 1):
            self._decoder.append(nn.Linear(d_model,
                                           d_model))
            self._decoder.append(activation())
            self._decoder.append(nn.LayerNorm(d_model))
        self.out_layer = nn.Linear(d_model,
                                   n_cls)
