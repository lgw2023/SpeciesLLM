...
class ExprDecoder(nn.Module):
    def __init__(
            ...
            use_cell_token_labels: bool = False,
    ):
        super().__init__()
        d_in = n_embd * 2 if use_cell_token_labels else n_embd
...
class MVCDecoder(nn.Module):
    def __init__(
        ...
        super().__init__()
        dim_in = n_embd * 2 if use_cell_token_labels else n_embd
...
class BERTForPreTraining(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.use_batch_labels:
            self.batch_encoder = CellTokenEncoder(config.num_batch_labels, config.hidden_size)
        if config.use_species_labels:
            self.species_encoder = CellTokenEncoder(config.num_species_labels, config.hidden_size)
        if config.use_tissue_labels:
            self.tissue_encoder = CellTokenEncoder(config.num_tissue_labels, config.hidden_size)
        if config.use_seqmethod_labels:
            self.seqmethod_encoder = CellTokenEncoder(config.num_seqmethod_labels, config.hidden_size)
        if config.use_disease_labels:
            self.disease_encoder = CellTokenEncoder(config.num_disease_labels, config.hidden_size)
        # Add sex and age encoders
        if config.use_sex_labels:
            self.sex_encoder = CellTokenEncoder(config.num_sex_labels, config.hidden_size)
        if config.use_age_labels:
            self.age_encoder = CellTokenEncoder(config.num_age_labels, config.hidden_size)
        ...
        self.decoder = ExprDecoder(
            ...
            use_cell_token_labels=True if any(cell_attributes) else False,
        )
        if config.do_mvc:
        ...
        if config.do_cls:
            self.cls_decoder = ClsDecoder(config.hidden_size, config.num_cls)
    ...
    def forward(
        ...
        outputs["cell_emb"] = torch.cat([cell_emb, cell_token_embs], dim=1) if isinstance(cell_token_embs, torch.Tensor) else cell_emb

        if MVC:
            mvc_output = self.mvc_decoder(
                torch.cat([cell_emb, cell_token_embs], dim = 1) if isinstance(cell_token_embs, torch.Tensor) else cell_emb,
