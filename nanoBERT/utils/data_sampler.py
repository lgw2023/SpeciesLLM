...
class SubsetBatchSampler(Sampler[List[int]]):
    r"""
        Samples batches of indices from a list of subsets of indices. Each subset of indices represents a data subset
        and is sampled without replacement randomly or sequentially. Specially, each batch only constains indices from
        a single subset. This sampler is for the scenaria where samples need to be drawn from multiple subsets separately.
    ...
    def __init__(self, subsets: List[Sequence[int]], batch_size: int, intra_subset_shuffle: bool = True,
            inter_subset_shuffle: bool = True, drop_last: bool = False, ):
        ...
        if intra_subset_shuffle:
            self.subsets_samplers = [SubsetRandomSampler(subset) for subset in subsets]
        else:
            self.subsets_samplers = [SubsetSequentialSampler(subset) for subset in subsets]

        self.batch_samplers = [BatchSampler(sampler,
                                            batch_size,
                                            drop_last) for sampler in self.subsets_samplers]
        ...
        if inter_subset_shuffle:
            ...
            self.batch_sampler_iterrators = [batch_sampler.__iter__() for batch_sampler in self.batch_samplers]
