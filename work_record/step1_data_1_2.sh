...
if [[ "${1:-}" != "--__speciesllm-step1-data-1-2-clean-env" ]]; then
  exec /usr/bin/env -i \
    HOME="${HOME:-/root}" \
    PATH="/data/miniconda3/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" \
    LANG=C.UTF-8 \
    bash "$0" --__speciesllm-step1-data-1-2-clean-env "$@"
fi
shift
...
usage() {
  cat <<'USAGE'
Usage:
  bash work_record/step1_data_1_2.sh [KEY=VALUE ...]
...
is_allowed_cli_var() {
  case "$1" in
    ENV_FILE|PYTHON_BIN|STAGE2_ROOT|INPUT_1ST|INPUT_2ND|\
...
export FIRST_VIEW=${FIRST_VIEW:-${STAGE2_ROOT}/views_1_2/1st_no_human_mouse_${RUN_ID}}
export MERGED_DIR=${MERGED_DIR:-${STAGE2_ROOT}/data_1_2_merged_full_no_1st_human_mouse_${RUN_ID}}
export FLAT_DIR=${FLAT_DIR:-${STAGE2_ROOT}/data_1_2_flatten_data_full_no_1st_human_mouse_${RUN_ID}}
...
# usage:
# bash work_record/step1_data_1_2.sh BATCH_FILES=2048 WORKERS=32 SHUFFLE_BUCKETS=16
# bash work_record/step1_data_1_2.sh RUN_ID=20260716_120000 \
# SKIP_MERGE=1 SKIP_SYNC=1 SKIP_PARTITION_IF_EXISTS=1 \
# FLAT_DIR=/data/disk1/SpeciesLLM_obs/Stage2_SpeciesLLMData/data_1_2_flatten_data_full_no_1st_human_mouse_20260716_120000_external \
