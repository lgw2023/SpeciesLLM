...
if [[ "${1:-}" != "--__speciesllm-step1-data-1-3-clean-env" ]]; then
  exec /usr/bin/env -i \
    HOME="${HOME:-/root}" \
    PATH="/data/miniconda3/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" \
    LANG=C.UTF-8 \
    bash "$0" --__speciesllm-step1-data-1-3-clean-env "$@"
fi
shift
...
usage() {
  cat <<'USAGE'
Usage:
  bash work_record/step1_data_1_3.sh [KEY=VALUE ...]
...
is_allowed_cli_var() {
  case "$1" in
    ENV_FILE|PYTHON_BIN|STAGE2_ROOT|INPUT_1ST|INPUT_3SC|\
...
export STAGE2_ROOT=${STAGE2_ROOT:-/data/disk2/SpeciesLLM_obs/Stage2_SpeciesLLMData}
...
export FIRST_VIEW=${FIRST_VIEW:-${STAGE2_ROOT}/views_1_3/1st_no_human_mouse_${RUN_ID}}
export MERGED_DIR=${MERGED_DIR:-${STAGE2_ROOT}/data_1_3_merged_full_no_1st_human_mouse_${RUN_ID}}
export FLAT_DIR=${FLAT_DIR:-${STAGE2_ROOT}/data_1_3_flatten_data_full_no_1st_human_mouse_${RUN_ID}}
