...
# 500M formal pretraining launcher for one explicitly specified full-data
...
#   bash work_record/step3_model_500M.sh DATA_PATH=/data/disk1/SpeciesLLM_obs/Stage2_SpeciesLLMData/your_flatten_dir
#
#   # Launch one long-running training job.
#   bash work_record/step3_model_500M.sh ACTION=launch EXPERIMENT_NAME=data_1_3_stable DATA_PATH=/data/.../your_flatten_dir
#
#   # Collect/check one finished job. Reuse the same RUN_ID used for launch.
#   bash work_record/step3_model_500M.sh ACTION=collect EXPERIMENT_NAME=data_1_3_stable RUN_ID=20260515_120000 DATA_PATH=/data/.../your_flatten_dir
#
# Notes:
...
if [[ "${1:-}" != "--__speciesllm-step3-500m-clean-env" ]]; then
  exec /usr/bin/env -i \
    HOME="${HOME:-/root}" \
    PATH="/data/miniconda3/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" \
    LANG=C.UTF-8 \
    bash "$0" --__speciesllm-step3-500m-clean-env "$@"
fi
shift
RUN_RECORD_ARGS=("$@")
...
is_allowed_cli_var() {
  case "$1" in
    ...
    EMB_ROOT|EMB_PATH|MODEL_CONFIG_JSON|GENE_EMBEDDING_MODALITIES|GENE_EMBEDDING_SUFFIX|SKIP_DATA_SYNC|\
    ...
    GRAD_CLIP_HARD_RAW_NORM_LIMIT|MAX_TRAIN_STEPS|\
...
run_pretrain_3node() {
  ...
  echo "[INFO] GENE_EMBEDDING_SUFFIX=${GENE_EMBEDDING_SUFFIX:-<none>}"
...
MODEL_CONFIG_JSON=${MODEL_CONFIG_JSON:-${EMB_PATH}/args_2nd_run_500m.json}
GENE_EMBEDDING_SUFFIX=${GENE_EMBEDDING_SUFFIX:-}
...
OUT_PATH=${OUT_PATH:-${WORKDIR}/training_output_500m_${EXPERIMENT_NAME}_from_scratch_${RUN_ID}}
COMMAND_DIR=${COMMAND_DIR:-${STAGE2_ROOT}/pretrain_500m_${EXPERIMENT_NAME}_commands_${RUN_ID}}
LOG_SUBDIR=${LOG_SUBDIR:-torchrun_logs/500m_${EXPERIMENT_NAME}_${RUN_ID}}
